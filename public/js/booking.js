...
var Countries = new Array("Argentina", "Australia", "Austria", "Belgium", "Brazil", "Bulgaria", "Canada", "Chile", "China", "Colombia", "Costa Rica", "Croatia", "Czech Republic", "Denmark", "Ecuador", "Estonia", "France", "Finland", "Germany", "Greece", "Holland", "Honduras", "Hong Kong S.A.R.", "Hungary", "Iceland", "India", "Ireland", "Indonesia", "Italy", "Japan", "Latvia", "Luxembourg", "Macau S.A.R.", "Malaysia", "Mexico", "Netherlands", "New Zealand", "Norway", "Paraguay", "Peru", "Philippines", "Poland", "Portugal", "Romania", "Spain", "Singapore", "Slovakia", "Slovenia", "South Africa", "South Korea", "Sweden", "Switzerland", "Taiwan", "Thailand", "Ukraine", "United Kingdom", "United States", "Uruguay", "Venezuela", "Others");
$(document).ready(function () {
    var countriesByLetter = {};
    countries.forEach(function (country) {
        var firstLetter = country.name.charAt(0).toUpperCase();
        if (!countriesByLetter[firstLetter]) {
            countriesByLetter[firstLetter] = [];
        }
        countriesByLetter[firstLetter].push(country);
    });
    for (var key in countriesByLetter) {
        countriesByLetter[key].sort(function (a, b) {
    ...
    for (var key in countriesByLetter) {
        html += '<dl>';
        html += '<dt>' + key + '</dt>';
        countriesByLetter[key].forEach(function (country) {
            html += '<dd>' + country.name + ' <span>(' + country.dial + ')</span></dd>';
        });
        html += '</dl>';
    }
    $('#book_phone').html(html);
    $("#country_search").on("keyup", function () {
        var value = $(this).val().toLowerCase();
        $("#book_phone dl").each(function () {
            var $dl = $(this);
            var hasMatchingCountry = false;
...
; jQuery(document).ready(function ($) {
    var ww = $(window).width();
    $('.js-phone, .b1_ps, .js-guest, .k2_s, #country_search, #book_phone').on('touchstart click', function (e) {
        e.stopPropagation();
    });
    $('.js-guest').on('click', function () {
        var $formGroup = $(this).closest('.form-group');
        var $dropdown = $formGroup.find('.k2_s');
        $('.k2_s').not($dropdown).hide();
        $dropdown.toggle();
        if (ww < 992) {
            $('body').addClass('box-hidden');
    ...
    const getGuests = () => [...new Set($('input[name^="guest_"][name$="_services"]').map(function () {
        return $(this).attr('name').match(/\d+/)[0];
    }).get())];
    function updateSummaryBox() {
        ...
        getGuests().forEach(id => {
            const services = $(`[name="guest_${id}_services"]:checked`).closest('.s8_i');
            if (!services.length)
                return;
            let rows = '';
            ...
            $box.append(`
                <div class="k2_di">
                    <table>
                        <tr><th colspan="2">${gettext('Guest')} ${id}:</th></tr>
                        ${rows}
                    </table>
                </div>
            `);
                ...
                <div class="k2_di">
                    <table>
                        <tr>
                            ...
                            <td><strong>${total.toLocaleString('vi-VN')} VND</strong></td>
                        </tr>
                    </table>
                </div>
            `);
    ...
    function syncFromGuest1() {
        const others = getGuests().filter(id => id !== '1');
        $('[name="guest_1_services"]').each(function () {
            const val = $(this).val();
            const checked = $(this).prop('checked');
            others.forEach(id => {
                $(`[name="guest_${id}_services"][value="${val}"]`).prop('checked', checked).trigger('change');
            }
            );
        });
        updateSummaryBox();
    ...
    $('[name="guest_1_services"]').on('change', function () {
        if ($('#all').is(':checked')) {
            syncFromGuest1();
        }
    });
    $('input[type="checkbox"][name^="guest_"][name$="_services"]').on('change', function () {
        const guestId = $(this).attr('name').match(/\d+/)[0];
        const selected = $(`[name="guest_${guestId}_services"]:checked`);
        const labels = selected.closest('.s8_i').find('h3').map(function () {
            return $(this).text().trim();
        }).get();
        const $input = $('.js-guest').eq(guestId - 1);
        $input.val(labels.join(', '));
        var $formGroup = $(this).closest('.form-group');
        $formGroup.toggleClass('done', selected.length > 0);
        updateSummaryBox();
    });
    $('.js-phone').click(function () {
        ...
        $('.k2_ps').show();
        $('.k2_ps dd').click(function () {
            $('.k2_ps dd').removeClass('active');
            $(this).addClass('active');
            $('.js-phone').val($(this).find('span').text());
            $('.k2_pc').html($(this).html());
            $('.k2_ps').hide();
            if (ww < 992) {
                $('body').removeClass('box-hidden');
    ...
    $('#fromBook').validate({
        ...
        messages: {
            ...
            email: {
                ...
                email: gettext("Please input valid email address")
            },
        },
    ...
    $('#fromBook').on('submit', function (e) {
        let valid = true;
        getGuests().forEach(id => {
            if ($(`[name="guest_${id}_services"]:checked`).length === 0) {
                showAlert(gettext('Guest must select at least one service.'));
                valid = false;
                return false;
            }
        }
