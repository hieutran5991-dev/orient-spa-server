...
export const spaLocations: SpaLocation[] = [
  ...
  },
  {
    ...
    features: {
      en: ["Couple's treatment rooms", 'Aromatherapy specialists', 'Hot stone massage', 'Premium location'],
      vi: ['Phòng trị liệu cho cặp đôi', 'Chuyên gia thơm liệu', 'Massage đá nóng', 'Vị trí cao cấp'],
      ja: [
        'カップル専用トリートメントルーム',
        'アロマセラピー専門',
        'ホットストーンマッサージ',
  ...
  },
  {
    ...
    features: {
      en: ['Largest location', 'VIP treatment rooms', 'Full body treatments', 'Group bookings welcome'],
      vi: ['Chi nhánh lớn nhất', 'Phòng trị liệu VIP', 'Liệu pháp toàn thân', 'Chào đón nhóm khách'],
      ja: ['最大の店舗', 'VIPトリートメントルーム', '全身トリートメント', 'グループ予約歓迎']
    },
    openHours: {
      en: 'Daily: 10:00 AM - 10:00 PM',
      vi: 'Hàng ngày: 10:00 - 22:00',
...
export const promotionPackages: SpaService[] = [
  ...
  },
  {
    ...
    benefits: {
      en: ['Body scrub 40 minutes', 'Body massage 90 minutes', 'Unlimited choice for body massage types'],
      vi: [
        'Tẩy tế bào chết toàn thân 40 phút',
        'Massage toàn thân 90 phút',
        'Không giới hạn lựa chọn loại massage toàn thân'
      ],
      ja: ['ボディスクラブ40分', 'ボディマッサージ90分', 'ボディマッサージタイプの選択無制限']
    },
    image: '/images/promotions/1778e314640f2068c1a75a9074cfeea6.jpg',
    isPromotion: true,
    isHot: false,
  ...
  },
  {
    ...
    benefits: {
      ...
      ja: ['フットスクラブ10分', 'サウナ/ハーブバス20分', 'ボディマッサージ90分', 'Oriヘアウォッシュ60分']
    },
    image: '/images/promotions/1778e314640f2068c1a75a9074cfeea6.jpg',
    isPromotion: true,
    isHot: true,
...
export const getSpaLocationsByLocale = (_locale: Locale = 'en'): SpaLocation[] => {
  return spaLocations
}
...
export const getSpaServicesByLocale = (_locale: Locale = 'en'): SpaService[] => {
  return spaServices
}

export const getSpaServicesByCategory = (category: string, locale: Locale = 'en'): SpaService[] => {
  return spaServices.filter((service) => service.category[locale].toLowerCase().includes(category.toLowerCase()))
}

export const getPromotionServices = (_locale: Locale = 'en'): SpaService[] => {
  return promotionPackages
}

export const getRegularServices = (_locale: Locale = 'en'): SpaService[] => {
  return spaServices.filter((service) => service.isPromotion === false)
}

export const getHotServices = (_locale: Locale = 'en'): SpaService[] => {
